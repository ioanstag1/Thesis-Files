...
def train(config, train_loader, test_loader, fold, test_idx):                                        
    ...
    tensorboard_log_path = os.path.join(model_save_path, 'tensorboard')

    writer = SummaryWriter(log_dir=tensorboard_log_path, comment='_'+config.model_name+'_'+config.writer_comment+'_'+str(fold))
