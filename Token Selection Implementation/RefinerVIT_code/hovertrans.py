...
class TokenSelector(nn.Module):
    def __init__(self, H, H_prime, embed_dim, kernel_size=3, attn_drop=0., stride=1):
        ...
        self.conv_exp = nn.Conv2d(H, H_prime, 1, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(H_prime)

        # Convolution for local token selection applied separately to each head with parameter groups=H_prime
        self.depth_sep_conv = nn.Conv2d(H_prime, H_prime, kernel_size, stride, padding=kernel_size // 2, groups=H_prime, bias=False)
        self.bn2 = nn.BatchNorm2d(H_prime)

        # Linear projection to reduce the number of attention maps from H_prime to H
        self.reduce_conv = nn.Conv2d(H_prime, H, 1, 1, bias=False)
        self.bn3 = nn.BatchNorm2d(H)
        
        self.adapt_bn = nn.BatchNorm2d(self.H)
    ...
    def forward(self, A, V):
        ...
        A_expanded = self.relu(self.bn1(self.conv_exp(A)))  # [B, H_prime, T, T]

        # Apply grouped convolution and ReLU on the expanded attention maps
        A_local = self.relu(self.bn2(self.depth_sep_conv(A_expanded)))

        # Apply linear projection to reduce the number of attention maps back to the original H
        x = self.bn3(self.reduce_conv(A_local))
        ...
        output = (x @ V).transpose(1, 2).reshape(B, T, -1)
...
class TSViTBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, H_prime):
        ...
        self.token_selector = TokenSelector(num_heads, self.H_prime, embed_dim)

    def forward(self, x):
        q, k, v, attn_output_weights = self.attention(x)
        attn_local_selected, attn_mixed = self.token_selector(attn_output_weights, v)
...
class Block(nn.Module):
    def __init__(self, dim, words_in_sentence, patch_size, sentences, in_chans=3, num_heads=4, num_inner_heads=4,
        ...
        self.attn_in1 = TSViTBlock(embed_dim=dim * words, num_heads=num_inner_heads, H_prime=num_inner_heads * 6)
        self.attn_in2 = TSViTBlock(embed_dim=dim * words, num_heads=num_inner_heads, H_prime=num_inner_heads * 6)
        ...
        self.attn_out1 = TSViTBlock(embed_dim=dim * words_in_sentence, num_heads=num_heads, H_prime=num_heads * 6)
        self.attn_out2 = TSViTBlock(embed_dim=dim * words_in_sentence, num_heads=num_heads, H_prime=num_heads * 6)
    ...
    def forward(self, pixel_embed1, pixel_embed2, row_embed, column_embed, relative_pos=None):
        ...
        return pixel_embed1, pixel_embed2, row_embed, column_embed, [attn_map1, attn_map2, attn_map3, attn_map4 ]
