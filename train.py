...
def CLAS(A_p, A_q):
    """
    Compute the class similarity between two attention maps.
    Args:
        A_p: Attention map from layer p (B, H, T, T)
        A_q: Attention map from layer q (B, H, T, T)
    Returns:
        clas_loss: Class similarity loss
    """
    dis_fn = torch.nn.CosineSimilarity(dim=1)  # Cosine similarity across the heads dimension
    
    # Initialize cos_loss to accumulate cosine similarity for each head and token
    cos_loss = 0
    for h in range(A_p.shape[1]):  # Loop over heads
        for t in range(A_p.shape[2]):  # Loop over tokens
            cos_loss += dis_fn(A_p[:, h, :, t], A_q[:, h, :, t])
    
    return cos_loss.mean()  # Mean over the batch
...
def L_div(att_maps, B):
    ...
    if att_maps is not None:
        for l in range(B):
            cos_loss += CLAS(att_maps[l], att_maps[l + 4])
...
if __name__ == '__main__':
    ...
    for train_idx, test_idx in cv.split(train_set):
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, sampler=train_sampler, num_workers=32)
